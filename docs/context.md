...
Você poderá utilizar bases de dados reais disponíveis em fontes como:
...
Bases públicas municipais, estaduais ou federais.

Bases abertas de instituições, empresas ou organizações.

Bases próprias, desde que autorizadas e sem dados sensíveis.

A base escolhida deve ser informada no relatório. Você deverá apresentar a origem dos dados, o link ou referência da fonte, a data de acesso e uma breve descrição do conteúdo da base.

Dados totalmente fictícios não serão aceitos.

### 5. Critérios mínimos da base de dados

A base escolhida deverá ter, preferencialmente:

Pelo menos 300 registros.

Pelo menos 6 colunas úteis para análise.

Pelo menos uma variável numérica.

Pelo menos uma variável categórica.

Possibilidade de aplicar limpeza, transformação e análise exploratória.

Possibilidade de criar novas variáveis.

Presença de algum problema de qualidade, como valores ausentes, duplicidades, erros de formato, categorias despadronizadas, outliers ou dados inconsistentes.

Evite escolher uma base já totalmente limpa e pronta, pois isso prejudica o objetivo do projeto. O foco aqui é tratar dados, não apenas abrir uma planilha e dizer que está tudo certo.
...
A base deve ser real.

A fonte dos dados deve ser informada.

A base original deve ser preservada.

O dataset final deve ser diferente da base original.

Todas as decisões de tratamento devem ser justificadas.

O notebook ou script deve estar organizado e executável.

Não basta mostrar código; é necessário explicar o que foi feito.

Não basta gerar gráficos; é necessário interpretar os resultados.

Não serão aceitos trabalhos apenas teóricos.

Não serão aceitos datasets tratados sem documentação.
